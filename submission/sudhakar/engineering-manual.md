...
Engineering review copy | 17 September 2026

Simulated observations demonstrate the calculations. They do not establish plant performance or validate future operating conditions.
...
## Simulated history and date windows

The common demonstration as-of date is 17 September 2026, 17:00 in America/Edmonton. History starts on 1 January 2026. The four monitoring models have 6,233 hourly observations. Furnace history has 1,559 four-hour observations. Economics uses daily intervals and a final partial day. Values vary reproducibly with operating load, seasonal conditions, degradation and a simulated maintenance event.

Heater fuel and process duties are linked. Exchanger flows and temperatures respect the heat balance and correction-factor domain. Membrane feed, permeate and hydrogen composition are balanced. Blower temperature rise is consistent with its pressure ratio and simulated efficiency. These are constructed operating examples, not additional client requirements.

Monitor window averages use finite observations with equal weight. Missing data is excluded; charts retain timestamp gaps. Blower reference fitting uses the complete supplied history, so changing the display window does not retrain the baseline. Furnace projections begin at the last available pass observation and remain separate from measured history.

Economics history recalculates each interval from variable feed and simulated prices using the actual routing engine. Period totals integrate hourly sales, feed cost and operating cost over the selected overlap, using 330/365 availability. The final partial interval is included. The annual scenario remains a separate 7,920-hour estimate. Equipment utility signals are not automatically charged to refinery OPEX; there is no confirmed plant allocation model.

## Python engines and input mapping

Use Python 3.12 or newer. In the submission folder, install requirements.txt, then run a model engine with its corresponding examples/<model>.input.json. For example: python engines/air_blower_engine.py examples/air-blower.input.json. Run python -m unittest discover -s tests to verify all six engines and examples without a website or network connection.

Each engine exposes run(payload). The common request carries schemaVersion 1.0, model, source, timezone, config, parameters and a list of measurements. Each measurement has an ISO-8601 timestamp, values and optional quality flags. Input fields and units are listed in README.md and data/units.json; merged defaults are in data/defaults.json.

Supply explicit UTC offsets. Bad, stale and missing quality become null before calculation; simulated quality remains labelled. Duplicate observation times, invalid configuration and unsupported units in the adapter are rejected. Success returns result, units, effective config, warnings and errors; failure returns ok=false with a structured error. A missing result is not evidence of normal operation.

The AVEVA interface is provisional. No confirmed payload contract, endpoint or live connection was supplied. adapter.py maps caller-provided site tags to canonical model fields and checks units. Site extraction, quality-code conversion, synchronization and credentials remain integration work. No credentials or private client emails are included.

Parity fixtures were produced by the website's TypeScript calculations, then run through Python with identical inputs. Tests compare raw outputs within 1e-9 relative or absolute tolerance, exact discrete outputs and sampled displayed rounding at 1, 2 and 4 decimals. This numerical tolerance accounts for arithmetic ordering, not engineering uncertainty. See verification-report.md for executed coverage and remaining limitations.
