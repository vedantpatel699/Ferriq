...
## September 17 follow-up: usable YTD and standalone delivery

- User screenshots show old browser-local 2021/2025 reference datasets overriding published 2026 samples. Added an explicit current-YTD sample action, retaining configuration and prior saved versions rather than discarding imported data.
- Removed Crude to Profit's historical graph; annual reconciliation and selected-period financial tables remain.
- Furnace original bundle has dated 2024–2025 records, not measured 2026 data. Preserve all original records and timestamps. Generate 2026 simulation from complete recorded multivariate operating blocks with four-hour interpolation; mark block transitions as synthetic, not maintenance evidence. No alarm thresholds or engineering validation relaxed.
- Six standalone Python engines include defaults, sample input, model/reference data and Edmonton timezone rules. Repeated validation/mapping blocks are explicitly commented. No-argument execution writes an example result beside the file.
- Six model PDF manuals generated from the actual website manual content/formula elements; replace previous combined PDF. AVEVA remains provisional. Checks/deployment pending this follow-up.

Follow-up validation: 154/154 desktop/tablet workflows passed, including recovery of saved 2021 datasets while retaining blower/exchanger configuration. A further 12/12 browser checks confirmed each PDF snapshot's full text and formula strings match the website. All 133 existing unit tests passed; the added archived-history guard and affected suites passed (19 tests), bringing the suite to 134. Build passed; lint retains eight existing warnings. Python checks passed: 141 production cases, nine contract/parity test methods, six CLI examples and six isolated single-file runs without helper/data files or installed packages. Existing 175-case economics parity and 394,833 legacy comparisons passed. All 16 PDF pages were rendered and visually inspected. Source-dependent engineering limits and provisional AVEVA status are unchanged.
...
## Complete supporting-page removal and generic model identities

Source: September 17 user request following the direct Watchlist URL screenshot. This supersedes the sidebar-only correction above.

- Removed Watchlist, Database/change log, Settings, Help, and redundant dashboard/predictor landing implementations and routes. Removed unused summary cards, backup-import context action, history reader, legacy manual AST and its generator. Model configuration, manual tabs and local saves remain functional.
- Overview now contains only the six named model links in simple tiles; no conditions, metrics, equipment IDs or extra explanatory content.
- Removed the remaining Watchlist summary hook and sidebar status badges. The sidebar uses the same six-entry registry as Overview. Previously it calculated every model's entire YTD history on each workspace refresh, delaying lazy page display and reload after an offline save; all four reproducing desktop/tablet checks passed after removal, without changing timeouts or assertions. Model-page status and calculations are unchanged.
- Removed plant equipment metadata and the blower plant/historian mapping document. Replaced the remaining plant-tag example in the blower Python sources and standalone file with the descriptive input name. Canonical calculation fields and numeric inputs are unchanged.
- Added source/data/deliverable identifier regression and browser regressions for six tiles and seven unavailable old routes. Retained storage-failure coverage using Crude to Profit after deleting the Settings page; removed tests belonging solely to deleted pages.
- Updated the device-folder blower file and its ZIP entry, verifying all other archive entries were preserved. Six PDF manuals contain no plant identifiers; their website content is unchanged.
- Final local validation: build, all 135 unit tests, all 122 desktop/tablet browser tests, Python/TypeScript parity (175 economics cases / 59,584 comparisons and blower parity), all nine submission test methods / 141 cases, and standalone-generator freshness pass. Six PDF identifier scans pass. Lint retains eight existing warnings. Overview screenshot visually reviewed. Remote main matches the reviewed base before commit. After the authorized push, verify the exact-commit Pages workflow, deployed artifact contents, six live workflows/manuals and seven removed routes; record the outcome in the final handoff.
- Scope: current tracked files, delivered files and deployed application. Existing Git history is not rewritten. Earlier engineering source uncertainties and provisional AVEVA status are unchanged.
