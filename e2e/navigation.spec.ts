...
test("sidebar nav actually navigates to every linked page (no href=# / dead links)", async ({ page }) => {
  await page.goto("/");
  const links = await page.locator("nav.sidebar a.navlink").all();
  expect(links.length).toBeGreaterThan(0);
  for (const link of links) {
    const href = await link.getAttribute("href");
    expect(href, "every sidebar nav link must have a real href, never #").not.toBe("#");
...
test("breadcrumb Home link on an equipment page returns to Home", async ({ page }) => {
