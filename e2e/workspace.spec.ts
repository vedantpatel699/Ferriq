...
test("all furnace passes, holdout data and manuals work", async ({
  ...
  await expect(page.getByRole("region",{name:"Observed operating driver",exact:true}).getByRole("img")).toBeVisible();
  await page.screenshot({path:testInfo.outputPath("furnace-validation.png")});
  await page
