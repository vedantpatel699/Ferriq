...
import {
  stoichO2KgPerKgFuel,
  stoichAirKgPerKgFuel,
  h2oKgPerKgFuel,
  calcCompositionProps,
  calcHeaterRow,
  buildHeaterAlerts,
  rollUpHeaterSeverity,
...
describe("Fired Heater — fuel composition stoichiometry", () => {
  ...
  it("stoichiometric O2/air/H2O per kg fuel for a ~94% methane blend", () => {
    ...
    expect(stoichAirKgPerKgFuel(natGas)).toBeCloseTo(16.4, 1);
...
describe("Fired Heater — calcHeaterRow internal energy-balance consistency", () => {
  ...
  it("qIn = qLhv + qFuelSensible + qCombustionAir, qAbs = qIn - qStack - qRad", () => {
    ...
    expect(result.qInKw as number).toBeCloseTo(
      result.qLhvKw! + result.qFuelSensibleKw! + result.qCombustionAirKw!,
      4,
    );
    expect(result.qAbsorbedKw as number).toBeCloseTo(
      result.qInKw! - result.qStackKw! - result.qRadiationLossKw!,
      4,
    );
  ...
  it("PTC4 indirect efficiency: 100 - dry - moist - rad - unacc", () => {
    ...
    expect(result.etaPtc4Pct as number).toBeCloseTo(
      100 -
        result.dryLossPct! -
        result.moistureLossPct! -
        result.radiationLossPct! -
        result.unaccountedLossPct!,
      4,
    );
  });
  ...
  it("bridgewall above alarm (870C) triggers alarm and rolls up severity", () => {
    ...
    expect(
      alerts.some(
        (a) => a.source === "tube metallurgy" && a.severity === "alarm",
      ),
    ).toBe(true);
  ...
  it("stack O2 below 2% (CO risk) is an alarm regardless of efficiency", () => {
    ...
    expect(
      alerts.some(
        (a) => a.source === "combustion safety" && a.severity === "alarm",
      ),
    ).toBe(true);
  });
});
