...
export interface MembraneConfig {
  designFeedH2Pct: number;
  designPermeateH2Pct: number;
  designRecoveryPct: number;
  designFlowNm3Hr: number;
  designRatio: number;
  designFeedPressureKpag: number;
  patmKpa: number;
  recoveryFloorPct: number;
  purityAdvisoryPct: number;
  purityAlarmPct: number;
  ratioAlarm: number;
  feedPressureDeviationPct: number;
}

export const DEFAULT_MEMBRANE_CONFIG: MembraneConfig = {
  designFeedH2Pct: 87.32,
  designPermeateH2Pct: 95.53,
  designRecoveryPct: 90.0,
...
export interface MembraneRowResult {
  ...
  permeateH2OnlinePct: number | null;
  permeateH2LabPct: number | null;
  feedH2OnlinePct: number | null;
  feedH2LabPct: number | null;
...
export function calcMembraneRow(r: MembraneRowInput): MembraneRowResult {
  ...
  const ratio =
    nonPermeate !== null && nonPermeate > 0 && r.feedFlowNm3Hr !== null
      ? r.feedFlowNm3Hr / nonPermeate
      : null;
  ...
  const recoveryOnlinePct =
    ...
    yFeedForOnline !== null &&
    yFeedForOnline > 0
      ? ((r.permeateFlowNm3Hr * r.permeateH2OnlinePct) /
          (r.feedFlowNm3Hr * yFeedForOnline)) *
        100
      : null;
  ...
  const recoveryLabPct =
    ...
    r.feedH2LabPct !== null &&
    r.feedH2LabPct > 0
      ? ((r.permeateFlowNm3Hr * r.permeateH2LabPct) /
          (r.feedFlowNm3Hr * r.feedH2LabPct)) *
        100
      : null;
...
export function synthesizeOnlineFeedH2(
  labSeries: (number | null)[],
): (number | null)[] {
  const samples: [number, number][] = [];
  labSeries.forEach((v, i) => {
    if (v !== null) samples.push([i, v]);
  });
  ...
  return labSeries.map((_, i) => {
    ...
    if (i <= samples[0][0]) val = samples[0][1];
      ...
      for (let s = 0; s < samples.length - 1; s++) {
        if (samples[s][0] <= i && samples[s + 1][0] >= i) {
          lo = samples[s];
          hi = samples[s + 1];
          break;
        }
...
export function buildMembraneAlerts(
  row: MembraneRowResult,
  cfg: MembraneConfig,
): EngineeringAlert[] {
  ...
  if (row.ratio !== null && row.ratio >= cfg.ratioAlarm) {
    alerts.push({
      severity: "alarm",
      message: `Total / Non-Permeate ratio ${row.ratio.toFixed(2)} at or above alarm ${cfg.ratioAlarm.toFixed(2)} - over-recovery, lower controller setpoint.`,
      source: "recovery ratio controller",
    });
  }
  if (
    row.recoveryOnlinePct !== null &&
    row.recoveryOnlinePct < cfg.recoveryFloorPct
  ) {
    alerts.push({
      severity: "advisory",
      message: `Online recovery ${row.recoveryOnlinePct.toFixed(2)} % below floor ${cfg.recoveryFloorPct.toFixed(0)} %.`,
      source: "recovery target",
    });
  }
  if (row.permeateH2OnlinePct !== null) {
    if (row.permeateH2OnlinePct < cfg.purityAlarmPct)
      alerts.push({
        severity: "alarm",
        message: `Permeate H2 ${row.permeateH2OnlinePct.toFixed(2)} % below alarm ${cfg.purityAlarmPct.toFixed(1)} %.`,
        source: "permeate analyzer",
      });
    else if (row.permeateH2OnlinePct < cfg.purityAdvisoryPct)
      alerts.push({
        severity: "advisory",
        message: `Permeate H2 ${row.permeateH2OnlinePct.toFixed(2)} % below advisory ${cfg.purityAdvisoryPct.toFixed(1)} %.`,
        source: "permeate analyzer",
      });
  }
  if (row.feedPressureKpag !== null) {
    const dev = Math.abs(
      ((row.feedPressureKpag - cfg.designFeedPressureKpag) /
    ...
    if (dev > cfg.feedPressureDeviationPct) {
      alerts.push({
        severity: "advisory",
        message: `Feed pressure ${Math.round(row.feedPressureKpag).toLocaleString()} kPag deviates ${dev.toFixed(2)} % from design.`,
        source: "feed pressure",
      });
    }
  }
  return alerts;
...
export function rollUpMembraneSeverity(
